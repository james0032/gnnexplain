"""
CompGCN Encoder - DGL Implementation

Learns both node and relation embeddings jointly using composition operations.
This is a DGL port for better batching and performance.
"""

import torch
import torch.nn as nn
import torch.nn.functional as F
import dgl
from .compgcn_layer_dgl import CompGCNConvDGL


class CompGCNEncoderDGL(nn.Module):
    """
    CompGCN Encoder for Knowledge Graphs (DGL version).

    Learns node and relation embeddings jointly through multi-layer
    composition-based graph convolutions.

    Args:
        num_nodes: Number of entities
        num_relations: Number of relations
        embedding_dim: Embedding dimension
        num_layers: Number of CompGCN layers
        comp_fn: Composition function ('sub', 'mult', 'corr')
        dropout: Dropout rate
        aggr: Aggregation scheme ('sum', 'mean', 'max')
    """

    def __init__(
        self,
        num_nodes: int,
        num_relations: int,
        embedding_dim: int,
        num_layers: int = 2,
        comp_fn: str = 'sub',
        dropout: float = 0.2,
        aggr: str = 'sum'
    ):
        super().__init__()

        self.num_nodes = num_nodes
        self.num_relations = num_relations
        self.embedding_dim = embedding_dim
        self.num_layers = num_layers
        self.comp_fn = comp_fn

        # Initial node embeddings
        self.node_emb = nn.Parameter(
            torch.Tensor(num_nodes, embedding_dim)
        )

        # Initial relation embeddings
        self.rel_emb = nn.Parameter(
            torch.Tensor(num_relations, embedding_dim)
        )

        # CompGCN layers
        self.convs = nn.ModuleList()
        for i in range(num_layers):
            self.convs.append(
                CompGCNConvDGL(
                    embedding_dim,
                    embedding_dim,
                    num_relations,
                    comp_fn=comp_fn,
                    aggr=aggr
                )
            )

        self.dropout = nn.Dropout(dropout)

        self.reset_parameters()

    def reset_parameters(self):
        """Initialize parameters with Xavier uniform."""
        nn.init.xavier_uniform_(self.node_emb)
        nn.init.xavier_uniform_(self.rel_emb)

    def forward(
        self,
        g: dgl.DGLGraph
    ) -> tuple:
        """
        Forward pass through CompGCN layers.

        Args:
            g: DGL graph with edge data 'etype' containing edge types

        Returns:
            Tuple of (node_embeddings, relation_embeddings)
        """
        x = self.node_emb
        rel = self.rel_emb

        # Pass through CompGCN layers
        for i, conv in enumerate(self.convs):
            x, rel = conv(g, x, rel)

            # Apply activation and dropout (except last layer)
            if i < len(self.convs) - 1:
                x = F.relu(x)
                x = self.dropout(x)

                rel = F.relu(rel)
                rel = self.dropout(rel)

        return x, rel

    def forward_with_edge_index(
        self,
        edge_index: torch.Tensor,
        edge_type: torch.Tensor
    ) -> tuple:
        """
        Forward pass using edge_index format (backward compatibility).

        Args:
            edge_index: Edge indices (2, num_edges)
            edge_type: Edge types (num_edges,)

        Returns:
            Tuple of (node_embeddings, relation_embeddings)
        """
        # Convert edge_index to DGL graph
        g = dgl.graph((edge_index[0], edge_index[1]), num_nodes=self.num_nodes)
        g.edata['etype'] = edge_type

        # Infer edge direction if not provided
        # Assume first half forward, second half backward
        num_edges = g.num_edges()
        edge_direction = torch.cat([
            torch.zeros(num_edges // 2, dtype=torch.long, device=edge_index.device),
            torch.ones(num_edges - num_edges // 2, dtype=torch.long, device=edge_index.device)
        ], dim=0)
        g.edata['direction'] = edge_direction

        return self.forward(g)

    def get_embeddings(
        self,
        edge_index: torch.Tensor = None,
        edge_type: torch.Tensor = None,
        g: dgl.DGLGraph = None
    ) -> dict:
        """
        Get final embeddings as a dictionary.

        Args:
            edge_index: Edge indices (optional, for backward compatibility)
            edge_type: Edge types (optional, for backward compatibility)
            g: DGL graph (preferred)

        Returns:
            Dictionary with 'node_emb' and 'rel_emb'
        """
        if g is not None:
            node_emb, rel_emb = self.forward(g)
        elif edge_index is not None and edge_type is not None:
            node_emb, rel_emb = self.forward_with_edge_index(edge_index, edge_type)
        else:
            raise ValueError("Either provide DGL graph 'g' or edge_index + edge_type")

        return {
            'node_emb': node_emb,
            'rel_emb': rel_emb
        }

    def __repr__(self):
        return (f'{self.__class__.__name__}('
                f'num_nodes={self.num_nodes}, '
                f'num_relations={self.num_relations}, '
                f'embedding_dim={self.embedding_dim}, '
                f'num_layers={self.num_layers}, '
                f'comp_fn={self.comp_fn})')
